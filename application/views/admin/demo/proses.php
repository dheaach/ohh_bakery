<?php
  ...
  if (is_array($pr) || is_object($pr)){
    ...
    foreach($page as $ud) {
      $onclick = "cae_prosesmnf('".$ud->pr_no."')";
      $records["data"][] = array(
      '<input class="chk-proses" type="checkbox" name="id[]" value="'.$ud->pr_no.'" onclick="checkProsesMnf()">',
      date('d-m-Y',strtotime($ud->pr_date)),
      '<a class="font-blue" data-toggle="modal" style="text-decoration:none;" onclick="'.$onclick.'" id="33C" >'.$ud->pr_no.'</a>',
      $ud->pr_ket,
      );
    }
  }
